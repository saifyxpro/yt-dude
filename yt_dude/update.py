...
def _get_variant_and_executable_path():
    ...
    if getattr(sys, 'frozen', False):
        ...
        # See: https://github.com/yt-dude/yt-dude/issues/11813
...
_NON_UPDATEABLE_REASONS = {
    ...
    **dict.fromkeys(
    ...
    'py2exe': 'py2exe is no longer supported by yt-dude; This executable cannot be updated',
    'source': 'You cannot update when running from source code; Use git to pull the latest changes',
    'unknown': 'You installed yt-dude from a manual build or with a package manager; Use that to update',
    'other': 'You are using an unofficial build of yt-dude; Build the executable again',
}
...
def _get_binary_name():
    return format_field(_FILE_SUFFIXES, detect_variant(), template='yt-dude%s', ignore=None, default=None)
...
def _get_outdated_warning():
    # Only yt-dude guarantees a stable release at least every 90 days
    if not ORIGIN.startswith('yt-dude/'):
        return None
    ...
    with contextlib.suppress(Exception):
        ...
        if last_updated < dt.datetime.now(dt.timezone.utc).date() - dt.timedelta(days=90):
            return ('\n         '.join((
                f'Your yt-dude version ({__version__}) is older than 90 days!',
                'It is strongly recommended to always use the latest version.',
                f'{is_non_updateable() or """Run "yt-dude --update" or "yt-dude -U" to update"""}.',
...
class Updater:
    ...
    def __init__(self, ydl, target: str | None = None):
        ...
        if '/' in self.requested_channel:
            ...
            if not self.requested_repo.startswith('yt-dude/') and self.requested_repo != self._origin:
    ...
    def _process_update_spec(self, lockfile: str, resolved_tag: str):
        ...
        for line in lines:
            ...
            if re.match(pattern, self._identifier):
                ...
                self._report_error(
                    f'yt-dude cannot be updated to {resolved_tag} since you are on an older Python version '
    ...
    def update(self, update_info=NO_DEFAULT):
        """Update yt-dude executable to the latest version
        ...
        self.ydl.to_screen(f'Updated yt-dude to {update_label}')
        return True
    ...
    def cmd(self):
        ...
        if not argv and getattr(sys, 'frozen', False):
            argv = sys.argv
        # linux_static exe's argv[0] will be /tmp/staticx-NNNN/yt-dude_linux if we don't fixup here
    ...
    def _block_restart(self, msg):
        def wrapper():
            self._report_error(f'{msg}. Restart yt-dude to use the updated version', expected=True)
...
def run_update(ydl):
    ...
    """
    deprecation_warning(
        '"yt_dude.update.run_update(ydl)" is deprecated and may be removed in a future version. '
        'Use "yt_dude.update.Updater(ydl).update()" instead')
