...
from yt_dude.dependencies import yt_dude_ejs as _has_ejs
from yt_dude.extractor.youtube.jsc._builtin import vendor
...
from yt_dude.extractor.youtube.pot._provider import configuration_arg
from yt_dude.extractor.youtube.pot.provider import provider_bug_report_message
from yt_dude.utils import version_tuple
from yt_dude.utils._jsruntime import JsRuntimeInfo

if _has_ejs:
    import yt_dude_ejs.yt.solver

TYPE_CHECKING = False
if TYPE_CHECKING:
    from collections.abc import Callable, Generator

    from yt_dude.extractor.youtube.jsc.provider import JsChallengeRequest

_EJS_WIKI_URL = 'https://github.com/yt-dude/yt-dude/wiki/EJS'
...
class EJSBaseJCP(JsChallengeProvider):
    ...
    def _pypackage_source(self, script_type: ScriptType, /) -> Script | None:
        if not _has_ejs:
            return None
        try:
            code = yt_dude_ejs.yt.solver.core() if script_type is ScriptType.CORE else yt_dude_ejs.yt.solver.lib()
        except Exception as e:
            self.logger.warning(
                f'Failed to load challenge solver {script_type.value} script from python package: {e}{provider_bug_report_message(self)}')
            return None
        return Script(script_type, ScriptVariant.MINIFIED, ScriptSource.PYPACKAGE, yt_dude_ejs.version, code)
