...
class AENetworksBaseIE(ThePlatformIE):  # XXX: Do not subclass from concrete IE
    ...
    def _extract_aetn_info(self, domain, filter_key, filter_value, url):
        ...
        if filter_key == 'canonical':
            ...
            graphql_video_id = (
                self._search_regex(
                    r'<meta\b[^>]+\bcontent="[^"]*\btpid/(\d+)"', webpage, 'id', default=None)
                or self._html_search_meta('videoId', webpage, 'GraphQL video ID', default=None)
                or self._search_regex(r'"videoId"\s*:\s*"([^"]+)"', webpage, 'escaped video ID', default=None)
            )
            if not graphql_video_id and re.match(r'https?://www\.', url):
                play_url = re.sub(r'^(https?://)www\.', r'\1play.', url)
                webpage = self._download_webpage(play_url, filter_value, note='Retrying with play.* subdomain')
                graphql_video_id = (
                    self._search_regex(
                        r'<meta\b[^>]+\bcontent="[^"]*\btpid/(\d+)"', webpage, 'id', default=None)
                    or self._html_search_meta('videoId', webpage, 'GraphQL video ID', fatal=True)
                )
        else:
